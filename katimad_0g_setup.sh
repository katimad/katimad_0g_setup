...
echo "🔑🔑바로 위에 Evmos 지갑 정보(주소,이름,펍키,시드문구) 출력완료!! 터미널마다 복사방법이 다른데🔑푸티의경우는 마우스 블록 지정한후 🔑 컨트롤 + shift +C 를 누르고 복사하면 되요 🔑🔑"
echo "🔑🔑Xshell은 블록 지정 후🔑 마우스 오른쪽클릭 + 복사 클릭🔑 ,일단 마우스 오른쪽 눌러보고 아무것도 안나오면, 자신이 쓰는 다양한 터미널을 네이버에 복사 방법 검색한후에 진행해. 🔑🔑."
read -p "🔑🔑자신없으면 폰으로 사진찍으세요.🔑복사 한것들은 메모장에 복사하셈🔑자 준비 되었으면 지금 엔터를 누르세요."
...
# 주소 출력되면 복사 하세요
read -p "🔑🔑바로위 펀셋 받을 메타마스크 주소를 출력 완료!! 주소를 꼭 복사해둬.🔑🔑푸티 터미널에서 복사는 컨트롤 + shift + C 입니다. 다른 터미널은 마우스로 블럭지정 후에 마우스오른쪽 클릭해봐🔑🔑자 완료 단계 가보자 지금 엔터를 누르세요🔑🔑 "
...
echo "설치완료 했고, 텔레 포스팅 2번부터 펀셋만 받아 검사기 실행만 하면되~ 채널 포스팅에 적어두었으니 한번더 확인하세요..안뇽"
read -p "🔑🔑아 요즘 펀셋 사이트가 한번씩 고장나거든. 디코에 공지확인하고. 정상화 되면 펀셋 받아 진행해 친구들~."
...
echo "🔑🔑아참참 현재 🔑🔑로그가 올라오고🔑🔑 블록이 잘 쌓이고 있는지 보여줄께🔑🔑 글들이 잘올라오고 움직이고 있으면 성공한거야.. 다보고 나면🔑🔑 컨트롤 + C 누르면되, 🔑자 보러 가보자 🔑🔑🔑🔑"
echo "🔑🔑 컨트롤 + C 기억해!! 🔑 만약 종료가 안되면 그냥 끄면되 알았지? VPS 라면 그냥 컴퓨터 끄면되.🔑 로그 잘올라오면 카티마드 만세 한번 해봐.🔑수고했어. 진짜 안녕!!"
